...
class Pointing(Genesys_Class):
    ...
    def set_initial_pointing_vector(self):
        ...
        q_rot_x = pu.gen_rotation_quat(total_pos_rad[0], self.axis_boresight_rot_x)
        q_rot_y = pu.gen_rotation_quat(total_pos_rad[1], -1.0*y_axis)
    ...
    def get_pointing_and_pol_angle(self, segment):
        """
        This is called for each new segment
        The simulation times, quaternions are set accordingly for the particular segment
        """
        t_steps = self.get_t_steps(segment)
        self.total_rotation_quaternion = self.generate_quaternion(t_steps)
        v_observed = self.rotate_vector(self.total_rotation_quaternion, self.pointing_vector_initial)
        pol_angle = self.get_pol_ang(t_steps, v_observed)

    def get_t_steps(self, segment):
        delta_t = 1.0 / self.params['sampling_rate']
        t_start = (segment - 1) * self.params['segment_length']
        t_stop = segment  * self.params['segment_length']
        self.n_samples = int(math.ceil(self.params['segment_length'] / delta_t))
        t_steps = t_start + delta_t * np.arange(self.n_samples)
        return t_steps

    def generate_quaternion(self, t_steps):
        w_rev = 2 * np.pi / self.params['t_year']
        w_prec = 2 * np.pi / self.params['t_precession']
        w_spin = 2 * np.pi / self.params['t_spin']
        ...
        if self.params['coordinate_system'] == 'galactic':
    ...
    def get_pol_ang(self, t_steps, v_observed):
        if self.params['polarisation_modulation'] == 'scan':
            ...
            HWP_angular_speed = 2*np.pi * self.params['WHP']['rpm'] / 60.0         # RPM to radians/sec
            pol_angle = (t_steps*HWP_angular_speed) % (2*np.pi)
        return pol_ang
