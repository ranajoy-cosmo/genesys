...
class Sky_Map:
    ...
    Masked map mask convention :
        ...
        True -> Pixel not seen
    """
    def __init__(self, map_obj=None, map_file_name=None, map_np=None, fields=None, map_labels=None):
        """
        Constructor
        Order of preference:
            map_obj -> copy_map(map_obj) : Copy constructor
        ...
        """
        if map_obj != None:
            self.copy_map(map_obj=map_obj)
        elif map_file_name != None:
            self.read_map_from_file(map_file_name=map_file_name, fields=fields, map_labels=map_labels)
        elif map_np != None:
            self.from_np_array(map_np, map_labels=map_labels)
        else:
            self.sky_map = None
            self.sky_map_labels = {}
            self.nside = None
    ...
    def copy_map(self, other):
        ...
        Copy constructor
        """
        self.sky_map = copy.deepcopy(other.sky_map)
        self.sky_map_labels = copy.deepcopy(other.sky_map_labels)
        self.nside = copy.deepcopy(other.nside)

    def read_map_from_file(self, map_file_name, fields, map_labels):
        """
        Read map from file
        The base directory of all maps is assumed to be global_config.maps_dir
        """
        self.sky_map = hp.read_map(os.path.join(global_paths.maps_dir, map_file_name), fields=fields)
        self.map_labels = map_labels
        self.nside = hp.get_nside(self.sky_map)

    def from_np_array(self, map_np, map_labels):
        """
        Accepts a Healpix map in numpy array format
        """
        self.sky_map = map_np
        self.sky_map_labels = map_labels
        self.nside = hp.get_nside(self.sky_map)
    ...
    def write_map(self, map_file_name):
        ...
        Writes map to file
        """
        hp.write_map(os.path.join(global_paths.map_dir, map_file_name), self.sky_map)
